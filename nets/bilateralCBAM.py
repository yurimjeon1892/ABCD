...
class BilateralConvCBAM(nn.Module):
    def __init__(self,
                 d, neighborhood_size,
        ...
        """
        super(BilateralConvCBAM, self).__init__()
        ...
        for idx, n_out_channel in enumerate(num_output[:-1]):
            ...
            sequential_list.append(
                nn.Conv2d(n_in_channel, n_out_channel, kernel_size=kernel_size, stride=1, padding=0, bias=True)
            )
        ...
        if not self.last_relu:
            ...
            sequential_list.append(
                nn.Conv2d(n_in_channel, num_final_output,
                          kernel_size=kernel_size,
                          stride=1, padding=0, bias=True)
            )
            if use_leaky:
                sequential_list.append(nn.LeakyReLU(0.1, inplace=True))
            else:
                sequential_list.append(nn.ReLU(inplace=True))

        ## add CBAM!
        sequential_list.append(ChannelGate(num_final_output))
        sequential_list.append(SpatialGate())

        self.blur_conv = nn.Sequential(*sequential_list)

        # initialize the weights
        for m in self.blur_conv.modules():
            init_weights(m)
